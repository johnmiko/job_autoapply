from autoapply.linkedin.answers import PhraseSimilarity


def test_phrase_similarity_works_with_string_input():
    phrase = "apple pie"
    phrase_list = ["apple", "apple cider", "banana", "apple pie with ice cream"]
    phrase_similarity = PhraseSimilarity(phrase, phrase_list)
    closest_phrase = phrase_similarity.find_closest_phrases()
    assert closest_phrase == "apple"
    assert phrase_similarity.similarities == [
...
def test_phrase_similarity_works_with_list_input():
    ...
    phrase_similarity = PhraseSimilarity(phrases, phrase_list)
    closest_phrase = phrase_similarity.find_closest_phrases()
